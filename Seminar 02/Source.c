...
void afisareAvion(struct Avion avion) {
	printf("%s are un avion de %d locuri cu pretul de %5.2fRON pe loc\n", avion.companie,
		avion.nrPasageri, avion.pretBilet);
}
...
struct Avion* copiazaPrimeleNAvioane(struct Avion* avioane, int nrAvioane, int nrAvioaneCopiate) {
	if (nrAvioaneCopiate <= nrAvioane && nrAvioaneCopiate > 0) {
		...
		for (int i = 0; i < nrAvioaneCopiate; i++) {
			avioaneCopiate[i] = initializareAvion(avioane[i].nrPasageri, avioane[i].companie,
				avioane[i].pretBilet);
...
void copiazaAvioaneCuBileteReduse(struct Avion* avioane, int nrAvioane, float pragPret,
	...
	//transmitere vector prin adresa (**)
	if (*avioaneNou != NULL) {
	...
	*avioaneNou = (struct Avion*)malloc(sizeof(struct Avion) * (*dimensiune)); //de intr
	int k = 0;

	for (int i = 0; i < nrAvioane; i++) {
		if (avioane[i].pretBilet < pragPret) {
			(*avioaneNou)[k++] = initializareAvion(avioane[i].nrPasageri, avioane[i].companie,
				avioane[i].pretBilet);
		}
	}
}

struct Avion getPrimulAvionDupaCompanie(struct Avion* avioane, int nrAvioane,
	const char* companie) {
	if (avioane != NULL && companie != NULL) {
		for (int i = 0; i < nrAvioane; i++) {
			if(avioane[i].companie != NULL){
				if (strcmp(avioane[i].companie, companie) == 0) {
					return initializareAvion(avioane[i].nrPasageri, avioane[i].companie,
						avioane[i].pretBilet);
				}
			}
		}
...
void main() {
	//vector - ocupa o zona de memorie(liniara) contigua
	//int v[10] - stockata pe stack, ocupa 10 * 4 biti
	//int* v - se aloca in runtime, in heap
	...
	avioane[4].pretBilet = 20;

	copiazaAvioaneCuBileteReduse(avioane, nrAvioane, 50, &avioaneCuBileteReduse,
