...
void citesteMatriceFisier(const char* nume_fisier, struct Locuinta** matrice, int nr_linii,
	int* nr_coloane) {
	if (nume_fisier != NULL && strlen(nume_fisier) > 0) {
		FILE* f = fopen(nume_fisier, "r");
		if (f != NULL) {
...
struct Locuinta cautareLocuintaDupaID(int idCautat, struct Locuinta** matrice, int nr_linii,
	int* nr_coloane)
{
	for (int i = 0; i < nr_linii; i++)
	{
	...
	struct Locuinta nullLocuinta;
	nullLocuinta.id = -1;
	nullLocuinta.numar = -1;
	nullLocuinta.strada = NULL;
...
void schimbareNumarLocuinta(struct Locuinta** matrice, int nr_linii, int* nr_coloane,
	int numarLocuinta)
{
